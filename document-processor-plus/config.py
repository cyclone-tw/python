...
try:
    ...
    print(f"[OK] 已載入環境變數檔案: {env_path}")
except ImportError:
    print("[WARNING] python-dotenv 未安裝，將使用系統環境變數或預設值")
...
if not GEMINI_API_KEY:
    print("[WARNING] 未設定 GEMINI_API_KEY！請在 .env 檔案中設定你的 API Key")
...
SETTINGS = {
    ...
    'notion': {
        ...
        'parent_page_id': '',  # 父頁面 ID（資料庫將建立在此頁面下）
        'task_db_id': '',      # 任務資料庫 ID
        'archive_db_id': '',   # 歸檔資料庫 ID
        'auto_sync': True,     # 是否自動同步
        'sync_on_calendar': True,   # 建立日曆事件時同步
        'sync_on_task': True,       # 建立任務時同步
        'sync_on_archive': True     # 歸檔時同步
    }
}
